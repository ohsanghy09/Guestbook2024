...
public class GuestbookServiceImpl implements  GuestbookService{
    ...
    public PageResultDTO<GuestbookDTO, Guestbook> getList(PageRequestDTO requestDTO) {
        Pageable pageable = requestDTO.getPageable(Sort.by("gno").descending()); //내림차순 정렬
        // 검색 기능 추가
        BooleanBuilder booleanBuilder = getSearch(requestDTO); // where절의 조건식
        Page<Guestbook> result = repository.findAll(booleanBuilder, pageable); // 조건식이 포함된 select문 생성
    ...
    public void modify(GuestbookDTO dto) {
        ...
        if(result.isPresent()){
            ...
            repository.save(entity); // 글의 제목과 내용을 업데이트 하는 문장이 실행된다.
        }
    }
